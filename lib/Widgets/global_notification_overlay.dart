...
class _GlobalNotificationOverlayState extends State<GlobalNotificationOverlay>
  ...
  void initState() {
    ...
    _successSubscription = BackgroundGenerationService().onGenerationComplete
        .listen((asset) {
          _enqueueNotification(asset: asset);
        });

    // Listen for failure
    _failureSubscription = BackgroundGenerationService().onGenerationFailure
        .listen((msg) {
                                  ...
                                  Text(
                                    _errorMessage ??
                                        (_currentAsset?.prompt ??
                                            'Your creation is ready to view'),
                                    maxLines: 1,
