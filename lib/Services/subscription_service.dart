...
class SubscriptionService {
  ...
  Future<void> initialize() async {
    ...
    if (apiKey.isEmpty) {
      debugPrint('⚠️ [SubscriptionService] RevenueCat API Key is empty. Configuration skipped.');
      return;
    }

    try {
      await Purchases.setLogLevel(kDebugMode ? LogLevel.debug : LogLevel.error);
      debugPrint('🛒 [SubscriptionService] FULL API KEY FROM FIREBASE: [$apiKey]');
      debugPrint('🛒 [SubscriptionService] API Key (first 5): ${apiKey.substring(0, 5.clamp(0, apiKey.length))}...');
      ...
      debugPrint('🛒 [SubscriptionService] Local cache loaded - subscribed: $_isSubscribed');

      // 0. Force refresh cache (important if entitlements were just created)
      debugPrint('🛒 [SubscriptionService] Invalidating cache to force-fetch latest metadata...');
      ...
      try {
        debugPrint('🛒 [SubscriptionService] Pre-fetching offerings to sync metadata...');
        _offerings = await Purchases.getOfferings();
        debugPrint('🛒 [SubscriptionService] Offerings pre-fetched: ${_offerings?.current?.identifier}');
      } catch (offeringsError) {
        debugPrint('⚠️ [SubscriptionService] Offerings pre-fetch failed (non-fatal): $offeringsError');
      ...
      Purchases.addCustomerInfoUpdateListener((customerInfo) {
        debugPrint('🛒 [SubscriptionService] CustomerInfo updated listener triggered.');
        _customerInfo = customerInfo;
        _updateSubscriptionStatus(customerInfo);
      });

      debugPrint('🛒 [SubscriptionService] Initialization complete.');
      debugPrint('🛒 [SubscriptionService] Current User ID: ${await Purchases.appUserID}');
    } catch (e) {
  ...
  Future<Offerings?> loadOfferings() async {
    try {
      ...
      _offerings = await Purchases.getOfferings();
      if (_offerings != null) {
        debugPrint('🛒 [SubscriptionService] Offerings loaded. Current: ${_offerings?.current?.identifier}');
        for (var o in _offerings!.all.values) {
          debugPrint('   Offering: ${o.identifier} has ${o.availablePackages.length} packages');
        }
      } else {
  ...
  Future<String?> buyPackage(Package package) async {
    try {
      debugPrint('🛒 [SubscriptionService] Starting purchase for package: ${package.identifier} (${package.packageType})');
  ...
  void _updateSubscriptionStatus(CustomerInfo customerInfo) async {
    ...
    debugPrint('   - All Entitlements in RC: ${customerInfo.entitlements.all.keys}');
    debugPrint('   - Active Entitlements in RC: ${customerInfo.entitlements.active.keys}');
    
    // Check for entitlements
    final EntitlementInfo? proEntitlement = customerInfo.entitlements.active[kEntitlementPro];
    final EntitlementInfo? ultraEntitlement = customerInfo.entitlements.active[kEntitlementUltra];
    ...
    debugPrint('🛒 [SubscriptionService] Status check: Pro=$isPro, Ultra=$isUltraTier -> shouldBeSubscribed=$shouldBeSubscribed');
    ...
    if (shouldBeSubscribed != _isSubscribed || isUltraTier != _isUltra) {
      ...
      if (_isSubscribed) {
        ...
        String? activeProductId = ultraEntitlement?.productIdentifier ?? proEntitlement?.productIdentifier;
        
        if (activeProductId != null) {
          int creditsToGrant = _getCreditsForProduct(activeProductId);
          await CreditService().setCredits(creditsToGrant);
          debugPrint('🛒 [SubscriptionService] Granted $creditsToGrant credits for product: $activeProductId');
        }
      }

      _subscriptionStreamController.add(_isSubscribed);
      debugPrint('🛒 [SubscriptionService] Status Updated — Subscribed: $_isSubscribed, Ultra: $_isUltra');
    }
  }
